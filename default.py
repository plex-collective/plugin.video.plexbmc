...
class PlexServers:
    ...
    def getURL(url, suppress=True, type="GET", popup=0):
        ...
        try:
            ...
            authHeader=MyPlexServers.getAuthDetails({'token':_PARAM_TOKEN}, False)
...
class MyPlexServers:
    ...
    def getMyPlexURL(url_path, renew=False, suppress=True):
        ...
        try:
            ...
            conn.request("GET", url_path+"?X-Plex-Token="+url_path.getMyPlexToken(renew))
            data = conn.getresponse()
...
class Sections:
    ...
    def getServerSections(ip_address, port, name, uuid):
        ...
        if not success:
            html=PlexServers.getURL('http://%s:%s/library/sections' % ( ip_address, port))
    ...
    def displaySections(filter=None, shared=False):
        ...
        if __settings__.getSetting('myplex_user') != '':
            GUI.addGUIItem('http://myplexqueue', {'title': 'myplex Queue'}, {'thumb': g_thumb, 'type': 'Video', 'mode': _MODE_MYPLEXQUEUE})
...
class OtherModes:
        ...
        for mediaserver in server_list.values():
            ...
            if type == "video":
                ...
                if number_of_servers == 1:
                    url.PlexPlugins(s_url+MyPlexServers.getAuthDetails(extraData,prefix="?"))
                    return
                ...
                if number_of_servers == 1:
                    url.plexOnline(s_url+MyPlexServers.getAuthDetails(extraData,prefix="?"))
                    return
                ...
                if number_of_servers == 1:
                    GUI.music(s_url+MyPlexServers.getAuthDetails(extraData,prefix="?"))
                    return
                ...
                if number_of_servers == 1:
                    GUI.photo(s_url+MyPlexServers.getAuthDetails(extraData,prefix="?"))
                    return
    ...
    def myPlexQueue():
        ...
        OtherModes.PlexPlugins('http://my.plexapp.com/playlists/queue/all', tree)
        return
    ...
    def channelView(url):
        ...
        for channels in tree.getiterator('Directory'):
            ...
            p_url=Utility.getLinkURL(url, {'key': channels.get('path',None), 'identifier' : channels.get('path',None)} , server)
...
class Commands:
    ...
    def alterAudio (url):
        ...
        audio_select_URL="http://%s/library/parts/%s?audioStreamID=%s" % ( Utility.getServerFromURL(url), part_id, audio_list[result] ) +MyPlexServers.getAuthDetails({'token':authtoken})
    ...
    def alterSubs (url):
        ...
        authtoken=MyPlexServers.getAuthTokenFromURL(url)
        sub_select_URL="http://%s/library/parts/%s?subtitleStreamID=%s" % ( Utility.getServerFromURL(url), part_id, sub_list[result] ) +MyPlexServers.getAuthDetails({'token':authtoken})
    ...
    def videoPluginPlay(vids, prefix=None, indirect=None):
        ...
        if '/services/url/lookup' in vids:
            ...
            if mediaCount > 1:
                ...
                if result == -1:
                    return
            videoPluginPlay(Utility.getLinkURL('',mediaDetails[result],server))
            return
        ...
        if indirect or '&indirect=1' in vids:
            ...
            for bits in tree.getiterator('Part'):
                videoPluginPlay(Utility.getLinkURL(vids,bits,server))
                break
    ...
    def PLAY(url):
        ...
        if url[0:4] == "file":
            ...
            if '?' in url:
                ...
                playurl=url+MyPlexServers.getAuthDetails({'token':_PARAM_TOKEN},prefix="?")
        else:
            playurl=url
    ...
    def monitorPlayback(id, server):
        ...
        while xbmc.Player().isPlaying():
            ...
            #If we are less than 95% completem, store resume time
                ...
                PlexServers.getURL("http://"+server+"/:/progress?key="+id+"&identifier=com.plexapp.plugins.library&time="+str(currentTime*1000),suppress=True)
                complete=0
            ...
            #Otherwise, mark as watched
                ...
                if complete == 0:
                    ...
                    PlexServers.getURL("http://"+server+"/:/scrobble?key="+id+"&identifier=com.plexapp.plugins.library",suppress=True)
                    complete=1
        ...
        if g_streamControl == _SUB_AUDIO_PLEX_CONTROL:
            ...
            if subtitle:
                ...
                try:
                    xbmc.Player().showSubtitles(False)
                    if subtitle.get('key'):
                        xbmc.Player().setSubtitles(subtitle['key']+MyPlexServers.getAuthDetails({'token':_PARAM_TOKEN},prefix="?"))
                    else:
    ...
    def playLibraryMedia(vids, override=0, force=None, full_data=False, shelf=False):
        ...
        if protocol == "file":
            ...
            if g_transcode == "true":
                ...
                playurl=PlexServers.transcode(id,url)+MyPlexServers.getAuthDetails({'token':_PARAM_TOKEN})
    ...
    def playPlaylist (server, data):
        ...
        tree = Utility.getXML(server+data['extra'].get('album')+"/children")
        if tree is None:
            return
...
class Media:
    ...
    def selectMedia(data, server):
        ...
        newurl=Media.mediaType({'key': options[result][0] , 'file' : options[result][1]},server,dvdplayback)
...
class GUI:
    ...
    def buildContextMenu(url, itemData):
        ...
        libraryRefresh = plugin_url+"update, " + refreshURL.split('?')[0]+MyPlexServers.getAuthDetails(itemData,prefix="?") + ")"
        context.append(('Rescan library section', libraryRefresh , ))

        #Mark media unwatched
        unwatchURL="http://"+server+"/:/unscrobble?key="+ID+"&identifier=com.plexapp.plugins.library"+MyPlexServers.getAuthDetails(itemData)
        unwatched=plugin_url+"watch, " + unwatchURL + ")"
        context.append(('Mark as Unwatched', unwatched , ))

        #Mark media watched
        watchURL="http://"+server+"/:/scrobble?key="+ID+"&identifier=com.plexapp.plugins.library"+MyPlexServers.getAuthDetails(itemData)
...
class Skin:
    ...
    def amberskin(container_id=None):
        ...
        for section in sections:
            ...
            extraData = {'fanart_image': Media.getFanart(section, section['address']), 'thumb': g_thumb}
            ...
            listitem.setProperty('partial', "ActivateWindow("+window+",plugin://plugin.video.plexbmc/?url=http://"+section['address']+section['path'])
            listitem.setProperty('search', "ActivateWindow(%s,plugin://plugin.video.plexbmc/?url=http://%s%s%s&mode=%s%s,return)" % (window, section['address'], section['path'], "/search?type=1", mode, aToken) )
            listitem.setProperty('recent', "ActivateWindow(%s,plugin://plugin.video.plexbmc/?url=http://%s%s%s&mode=%s%s,return)" % (window, section['address'], section['path'], "/recentlyAdded", mode, aToken) )
            listitem.setProperty('viewed', "ActivateWindow(%s,plugin://plugin.video.plexbmc/?url=http://%s%s%s&mode=%s%s,return)" % (window, section['address'], section['path'], "/recentlyViewed", mode, aToken) )
            listitem.setProperty('ondeck', "ActivateWindow(%s,plugin://plugin.video.plexbmc/?url=http://%s%s%s&mode=%s%s,return)" % (window, section['address'], section['path'], "/onDeck", mode, aToken) )
            listitem.setProperty('newest', "ActivateWindow(%s,plugin://plugin.video.plexbmc/?url=http://%s%s%s&mode=%s%s,return)" % (window, section['address'], section['path'], "/newest", mode, aToken) )
    ...
    def fullShelf(server_list={}):
        ...
        for index in recent_list:
            ...
            if media.get('type',None) == "movie":
                ...
                if __settings__.getSetting('hide_watched_recent_items') == 'false' or media.get("viewCount", 0) == 0:
                    m_url="plugin://plugin.video.plexbmc?url=%s&mode=%s&t=%s%s" % ( Utility.getLinkURL('http://'+server_address,media,server_address), _MODE_PLAYSHELF, randomNumber, aToken)
                ...
                s_url="ActivateWindow(VideoLibrary, plugin://plugin.video.plexbmc?url=%s&mode=%s%s, return)" % ( Utility.getLinkURL('http://'+server_address,media,server_address), _MODE_TVEPISODES, aToken)
                ...
                s_url="ActivateWindow(MusicFiles, plugin://plugin.video.plexbmc?url=%s&mode=%s%s, return)" % ( Utility.getLinkURL('http://'+server_address,media,server_address), _MODE_TRACKS, aToken)
                s_thumb=server_list.getShelfThumb(media,server_address,seasonThumb=0)+aToken
                ...
                s_url="ActivateWindow(Videos, plugin://plugin.video.plexbmc?url=%s&mode=%s%s, return)" % ( Utility.getLinkURL('http://'+server_address, media, server_address, season_shelf=True), _MODE_TVEPISODES, aToken)
                ...
                recentSeasonCount += 1
        server_list.clearShelf(recentMovieCount, recentSeasonCount, recentMusicCount, recentPhotoCount)
        ...
        for index in sorted(ondeck_list):
            ...
            if media.get('type',None) == "movie":
                ...
                s_url="PlayMedia(plugin://plugin.video.plexbmc?url=%s&mode=%s&t=%s%s)" % (Utility.getLinkURL('http://'+server_address, media, server_address), _MODE_PLAYSHELF, randomNumber, aToken)
    ...
    def shelf(server_list=None):
        ...
        for server_details in server_list.values():
            ...
            tree=Utility.getXML('http://'+server_details['server']+":"+server_details['port']+endpoint)
            if tree is None:
                xbmc.executebuiltin("XBMC.Notification(Unable to contact server: "+server_details['serverName']+",)")
                server_list.clearShelf()
                return
        ...
        for index in sorted(added_list, reverse=direction):
            ...
            if media.get('type',None) == "movie":
                ...
                s_url="PlayMedia(plugin://plugin.video.plexbmc?url=%s&mode=%s%s)" % ( Utility.getLinkURL('http://'+server_address,media,server_address), _MODE_PLAYSHELF, aToken)
                s_thumb="http://"+server_address+media.get('grandparentThumb','')
    ...
    def shelfChannel(server_list=None):
        ...
        for server_details in server_list.values():
            ...
            tree=Utility.getXML('http://'+server_details['server']+":"+server_details['port']+'/channels/recentlyViewed')
            if tree is None:
                xbmc.executebuiltin("XBMC.Notification(Unable to contact server: "+server_details['serverName']+",)")
            ...
            for media in tree:
                ...
                c_url="ActivateWindow(%s, plugin://plugin.video.plexbmc?url=%s&mode=%s%s)" % ( channel_window, Utility.getLinkURL('http://'+server_details['server']+":"+server_details['port'],media,server_details['server']+":"+server_details['port']), mode , aToken)
...
def jason_test():
    ...
    listitems.append((path, liz, True))  #False??? should maybe be set back to Fasle since not a folder
    xbmcplugin.addDirectoryItems(pluginhandle, listitems)
    xbmcplugin.endOfDirectory(handle=pluginhandle)
...
pluginhandle = None if not (len(sys.argv) >= 1 and sys.argv[1].isdigit()) else int(sys.argv[1])
...
#    pass
   ...
   #pluginhandle = int(sys.argv[1])
    ...
    if ( mode == None ) or ( param_url == None ) or ( len(param_url)<1 ):
        ...
        Commands.playLibraryMedia(param_url,force=force, override=param_transcodeOverride)
        ...
        Commands.videoPluginPlay(param_url,param_identifier,param_indirect)
